...
- AI chat pane (right side, toggled from the toolbar): ask about your data in a conversation. The assistant can run **read-only** SQL itself to look things up — writes are rejected even while the Writable switch is on — and every query it ran is shown with the answer. Code blocks have Copy / Insert buttons; rows it read are sent back to the model as tool results, so keep the pane closed when that is not acceptable
