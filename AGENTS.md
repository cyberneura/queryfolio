...
| db.rs | sqlx プール管理、クエリ実行・キャンセル (CancelRegistry)、型別 JSON 変換、readonly ガード / 危険な文ガード (dangerous_reason)。非 SQL エンジン (Redis / Elasticsearch) は `DbPool` の variant として持ち、run_query_cancellable の冒頭で engines/ の各モジュールへ委譲する |
